...
void TIM14_CH1_PWM_Init(u16 per,u16 psc)
{
	...
	TIM_OCInitStructure.TIM_OCPolarity=TIM_OCPolarity_Low;  //输出机极性
	TIM_OCInitStructure.TIM_Pulse=per/2;					//脉冲宽度
...
void TIM9_CH2_PWM_Init(u16 per,u16 psc)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM9,ENABLE);//使能TIM14时钟
	
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource3,GPIO_AF_TIM9);//管脚复用
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_AF; //复用输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_3;//管脚设置
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_100MHz;//速度为100M
	GPIO_InitStructure.GPIO_OType=GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_PuPd=GPIO_PuPd_UP;//上拉
	GPIO_Init(GPIOF,&GPIO_InitStructure); //初始化结构体
	...
	TIM_OCInitStructure.TIM_OCMode=TIM_OCMode_PWM1;
	TIM_OCInitStructure.TIM_OCPolarity=TIM_OCPolarity_High;  //输出机极性
	TIM_OCInitStructure.TIM_Pulse=10;					//脉冲宽度
	TIM_OCInitStructure.TIM_OutputState=TIM_OutputState_Enable;
	TIM_OC1Init(TIM9,&TIM_OCInitStructure); //输出比较通道1初始化
	
	TIM_OC1PreloadConfig(TIM9,TIM_OCPreload_Enable); //使能TIMx在 CCR1 上的预装载寄存器
	TIM_ARRPreloadConfig(TIM9,ENABLE);//使能预装载寄存器
