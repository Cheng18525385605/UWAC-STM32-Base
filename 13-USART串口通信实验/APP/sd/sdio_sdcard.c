  ...
  SDIO->DCTRL=0x0;	//数据控制寄存器清零(关DMA) 
	...
	if(DeviceMode==SD_DMA_MODE)
	{
		...
		SDIO->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<5)|(1<<9);	//配置需要的中断 
 		...
 		while((DMAEndOfTransfer == 0)&&(TransferError==SD_OK)&&timeout){
				timeout--;
				
				// 1. 检查 DMA 传输错误 (TEIF)
				if(DMA_GetFlagStatus(DMA2_Stream3, DMA_FLAG_TEIF3) != RESET)
				{
						DMA_ClearFlag(DMA2_Stream3, DMA_FLAG_TEIF3);
						return SD_DATA_TIMEOUT; // 返回错误代码
				}
				
				// 2. 检查 SDIO 硬件错误 (CRC失败, 超时, 上溢)
				// 如果 SDIO 出错，它不会通知 DMA 结束，所以必须手动检查
				if(SDIO->STA & (SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_RXOVERR))
				{
						SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_RXOVERR);
						return SD_DATA_CRC_FAIL; // 返回错误代码
				}
		}
...
SD_Error SD_ReadMultiBlocks(u8 *buf, long long addr, u16 blksize, u32 nblks)
{
    ...
    if (nblks > 1) 
    {
        ...
        {
            // ... (POLLING模式代码保持不变，此处省略以节省篇幅，直接使用您原有的逻辑) ...
            // 为了完整性，如果您需要POLLING代码，请保留原文件中的内容
            // 这里主要修复DMA部分
            INTX_DISABLE();
            while(!(SDIO->STA&((1<<5)|(1<<1)|(1<<3)|(1<<8)|(1<<9))))
            {
                if(SDIO_GetFlagStatus(SDIO_FLAG_RXFIFOHF) != RESET)
                {
                    ...
                    tempbuff+=8; timeout=0X7FFFFF;
                }
                else { if(timeout==0)return SD_DATA_TIMEOUT; timeout--; }
            }
            if(SDIO_GetFlagStatus(SDIO_FLAG_DTIMEOUT) != RESET) { SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT); return SD_DATA_TIMEOUT; }
            else if(SDIO_GetFlagStatus(SDIO_FLAG_DCRCFAIL) != RESET) { SDIO_ClearFlag(SDIO_FLAG_DCRCFAIL); return SD_DATA_CRC_FAIL; }
            else if(SDIO_GetFlagStatus(SDIO_FLAG_RXOVERR) != RESET) { SDIO_ClearFlag(SDIO_FLAG_RXOVERR); return SD_RX_OVERRUN; }
            else if(SDIO_GetFlagStatus(SDIO_FLAG_STBITERR) != RESET) { SDIO_ClearFlag(SDIO_FLAG_STBITERR); return SD_START_BIT_ERR; }
            while(SDIO_GetFlagStatus(SDIO_FLAG_RXDAVL) != RESET) { *tempbuff=SDIO->FIFO; tempbuff++; }
        ...
        else if (DeviceMode == SD_DMA_MODE)
        {
            ...
            TransferEnd = 0;        // 传输结束标置位
            DMAEndOfTransfer = 0;   // DMA传输标志清零
            
            SDIO->MASK |= (1 << 1) | (1 << 3) | (1 << 8) | (1 << 5) | (1 << 9); // 配置中断
            SDIO->DCTRL |= 1 << 3;                                              // SDIO DMA使能
            SD_DMA_Config((u32 *)buf, nblks * blksize, DMA_DIR_PeripheralToMemory);

            timeout = SDIO_DATATIMEOUT;
            // 【关键修复1】等待DMA完成，并加入错误检测
            while ((DMAEndOfTransfer == 0) && (TransferError == SD_OK) && timeout)
            {
                timeout--;
                // 检查DMA传输错误
                ...
                // 检查SDIO硬件错误 (如果SDIO出错，DMA不会完成，必须在这里拦截)
                if (SDIO->STA & (SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_RXOVERR))
                {
                    SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_RXOVERR);
                    return SD_DATA_CRC_FAIL;
            ...
            // 【关键修复2】DMA完成后，等待SDIO传输结束 (TransferEnd)
            // 注意：此时DMAEndOfTransfer已经是1了，不能放在while条件里
            timeout = SDIO_DATATIMEOUT;
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		SD_DMA_Config((u32*)buf,blksize,DMA_DIR_MemoryToPeripheral);				//SDIO DMA配置
 	 	SDIO->DCTRL|=1<<3;								//SDIO DMA使能.  
 		while((DMAEndOfTransfer == 0)&&timeout){
				timeout--;
				...
				// 检查 SDIO 错误 (写操作检查下溢 TXUNDERR 而不是上溢)
				if(SDIO->STA & (SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_TXUNDERR))
				{
						SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_TXUNDERR);
						return SD_DATA_CRC_FAIL;
				}
		}
		if(timeout==0)
		...
		timeout=SDIO_DATATIMEOUT;
		while((DMAEndOfTransfer == 0x00) && (TransferEnd==0)&&(TransferError==SD_OK)&&timeout)timeout--;
 		if(timeout==0)return SD_DATA_TIMEOUT;			//超时	 
  	if(TransferError!=SD_OK)return TransferError;
...
SD_Error SD_WriteMultiBlocks(u8 *buf, long long addr, u16 blksize, u32 nblks)
{
    ...
    if (nblks > 1)
    {
        ...
        if (DeviceMode == SD_POLLING_MODE)
        {
            ...
            while(!(SDIO->STA&((1<<4)|(1<<1)|(1<<8)|(1<<3)|(1<<9))))
            {
                ...
                {     
                    ...
                    else 
                    {
                        ...
                        tempbuff+=SD_HALFFIFO; bytestransferred+=SD_HALFFIFOBYTES;
                    }
                    timeout=0X3FFFFFFF;
                }
                else { if(timeout==0)return SD_DATA_TIMEOUT; timeout--; }
            ...
            else if(SDIO_GetFlagStatus(SDIO_FLAG_TXUNDERR) != RESET) { SDIO_ClearFlag(SDIO_FLAG_TXUNDERR); return SD_TX_UNDERRUN; }
            else if(SDIO_GetFlagStatus(SDIO_FLAG_STBITERR) != RESET) { SDIO_ClearFlag(SDIO_FLAG_STBITERR); return SD_START_BIT_ERR; }
            if(SDIO_GetFlagStatus(SDIO_FLAG_DATAEND) != RESET)
            {                                                            
                if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||(SDIO_STD_CAPACITY_SD_CARD_V2_0==CardType)||(SDIO_HIGH_CAPACITY_SD_CARD==CardType))
        ...
        else if (DeviceMode == SD_DMA_MODE)
        {
            ...
            timeout = SDIO_DATATIMEOUT;
            // 【关键修复1】等待DMA发送完成，并加入错误检测
            while ((DMAEndOfTransfer == 0x00) && timeout)
            {
                timeout--;
                // 检查DMA错误
                if (DMA_GetFlagStatus(DMA2_Stream3, DMA_FLAG_TEIF3) != RESET)
